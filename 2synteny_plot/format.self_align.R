...
setwd("F:/Myriapod/longest_protH/Orthofinder/Results_Jun10")

opt$input_filename="Orthogroups/Orthogroups.tsv"
species.pair.all<-read.delim(opt$input_filename, header = TRUE)

for (j in 2:ncol(species.pair.all)) {
  ...
  species.pair<-species.pair.all[-which(species.pair.all[,j] == ""),c(1,j,j)]
  species.name=names(species.pair.all)[j]
  ...
  species.pair.1v1<-subset(species.pair.1v1, gene1 != gene2)
  write.table(species.pair.1v1, file = paste(opt$output_filename,".",species.name,"__v__",species.name,".txt", sep = ""), sep = '\t', row.names = FALSE)
