...
Orthofinder_dir="F:/Myriapod/longest_protH/Orthofinder/Results_Jun10/"
...
##Species order
Species_tree_data<-read.table(file = paste(Orthofinder_dir,
                                           "CAFE/r8s_lambda1/Species_tree_gain_loss_dup.txt",
                                           sep = ""), 
                              header = TRUE, sep = "\t")
nspecies=11
...
heatmap.overlap.orthogroups<-ggplot(species.overlaps.1v1,
  ...
  labs(y = "", x = "", fill = "Ratio", title = "Overlap Orthogroups among Species")+
  theme(plot.subtitle = element_text(vjust = 1),
        plot.caption = element_text(vjust = 1),
        #axis.title = element_text(size = 0),
        axis.text.x = element_text(size = 11, colour = "black", angle = 45,
...
ggsave("heatmap.overlap.orthogroups.png", width = 10, height = 10, units = "in", dpi = 300)
ggsave("heatmap.overlap.orthogroups.tiff", width = 10, height = 10, units = "in", dpi = 300)
